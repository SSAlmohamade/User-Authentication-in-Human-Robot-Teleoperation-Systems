...
def Position_Velocity_similarity(self):
    ...
    fileID2 = Type+'Attacker_'+name+'_against_'+str(Targeted)+'_Attempt_'+str(Attacker)+'.csv'
    ...
    return PT_1,PT_2,PT_3,vm1_1,vm1_2,vm1_3,PT_1A, PT_2A, PT_3A, vm2_1A, vm2_2A,vm2_3A,DT1,DT2,DT3,DA1,DA2,DA3
...
def Targeted_plot(Tar):
    ...
    global Targeted
    Targeted = Tar
    fileID1 = 'Data/User_'+str(Targeted)+'_Ex2_Task2.csv'
    dataset = pd.read_csv(fileID1)
...
def plot_similarity(self,Att,Tar,Id,TP):
    global Attacker
    global Targeted
    global name
    global Type
    ...
    name=Id
    Type=TP
    fileID1 = 'Data/User_'+Targeted+'_Ex2_Task2.csv'
    fileID2 = Type+'Attacker_'+str(name)+'_against_'+str(Tar)+'_Attempt_'+str(Attacker)+'.csv'
    dataset = pd.read_csv(fileID1)
    dataset2 = pd.read_csv(fileID2)

    # result = profile_similarity(self)
    probabilityAll,result,prpba= Decision_seg()
    ...
    return probabilityAll, result , prpba
...
def Decision_seg():
    ...
    for A in Featuerfile:
        ...
        clf = OneClassSVM(nu=0.4).fit(tagetuser_SampleX)
    ...
    return probabilityAll,result , D
...
def Decision_seg_time():
    ...
    for A in Featuerfile:
        ...
        # clf = OneClassSVM(nu=0.7,max_iter=100,).fit(tagetuser_SampleX)
        # # print('OneClassSVM',clf.predict(X_test))
        # OneClassSVMAll.append(clf.predict(X_test)[0])
        #
        # RF = IsolationForest(n_estimators =200,max_features=X_test.shape[1],\
