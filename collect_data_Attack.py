...
def Save_Data(Data,name,num,Tar,Type):
        print('data', Data)
        Data.to_csv(Type+'Attacker_'+name+'_against_'+str(Tar)+'_Attempt_'+str(num)+'.csv',index=False)
...
def main (name,num,Tar,Type):
   ...
   df['Accel_M'] = Accel_M
   print('data', df)
   Save_Data(df,name,num,Tar,Type)
