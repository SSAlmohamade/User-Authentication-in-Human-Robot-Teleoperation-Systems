# -*- coding: utf-8 -*-
"""
Created on Tue Sep 20 10:19:43 2020
        ...
        if ((round(TPosition0[0], 1) == round(Sphere_P[0], 1)) and (round(Sphere_P[1], 1) == round(TPosition0[1], 1))):
            ...
            while not (end_maze):
                ...
                while not (Testpoint):
                    ...
                    errorCodeTarget, manipSphere = vrep.simxGetObjectHandle(clientID, 'Sphere',
                                                                            vrep.simx_opmode_oneshot_wait)
                    returnCode, SPosition = vrep.simxGetObjectPosition(clientID, manipSphere, -1,
                                                                       vrep.simx_opmode_oneshot_wait)
                    Sphere_P = [SPosition[0], SPosition[1]]

                    # get Velocity  from v-rep
                    errorCodeTarget, Sphere_Velocity = vrep.simxGetObjectHandle(clientID, 'Sphere',
                                                                                vrep.simx_opmode_oneshot_wait)
                    returnCode, linearVelocity, angularVelocity = vrep.simxGetObjectVelocity(clientID, Sphere_Velocity,
                    ...
                    if ((round(TPosition3[0], 1) == round(Sphere_P[0], 1)) and (
                            round(Sphere_P[1], 1) == round(TPosition3[1], 1))):
                        Testpoint = True
                        # print("testpoint")
                temp = []
                Testpoint2 = False
                ...
                while not (Testpoint2):
                    ...
                    if ((round(TPosition4[0], 1) == round(Sphere_P[0], 1)) and (
                            round(Sphere_P[1], 1) == round(TPosition4[1], 1))):
                        Testpoint2 = True
                ...
                while not (end_maze):
                    ...
                    if ((round(TPosition[0], 1) == round(Sphere_P[0], 1)) and (
                            round(Sphere_P[1], 1) == round(TPosition[1], 1))):
                        end_maze = True
                        print("End_maze")
...
def Save_Data(Data):
        print('data', Data)
        Data.to_csv(User_Name+'_Ex'+str(Experment)+'_Task'+str(Task_NO)+'.csv',index=False)
